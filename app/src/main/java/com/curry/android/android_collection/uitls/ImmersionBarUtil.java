...
/**
 * @author curry
 * created at 2018/4/9 14:29
 * @desc 动态计算沉浸式状态栏的高度--工具类
 *
 * 使用此沉浸式状态栏,当activity的底部有edittext时,需要在布局中嵌套ScrollView和SoftKeyInputUtil.assistActivity(this);
 */
public class ImmersionBarUtil {
    ...
    //侧滑菜单页面的内容视图
    private int mContentResoureIdInDrawer;

    public ImmersionBarUtil(Activity activity) {
        mActivity = activity;
    }

    public static ImmersionBarUtil newInstance(Activity activity) {
    ...
    public ImmersionBarUtil setColor(int color) {
        mColor = color;
        return this;
    }
    ...
    public ImmersionBarUtil setDrawable(Drawable drawable) {
        mDrawable = drawable;
        return this;
    }
    ...
    public ImmersionBarUtil setIsActionBar(boolean actionBar) {
     ...
     * @return
     */
    public ImmersionBarUtil setDrawerLayoutContentId(boolean drawerLayout, int contentId) {
        mIsDrawerLayout = drawerLayout;
        mContentResoureIdInDrawer = contentId;
        return this;
    }
    ...
    public void init() {
        ...
        if (mDrawable != null) {
            ...
            addStatusViewWithDrawable(mActivity, mDrawable);
        }
        if (isDrawerLayout()) {
     ...
     * 去除 ActionBar 阴影
     */
    public ImmersionBarUtil clearActionBarShadow() {
    ...
    private void addStatusViewWithColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDrawerLayout()) {
                ...
                DrawerLayout drawer = (DrawerLayout) parentView;
                //内容视图
                View content = activity.findViewById(mContentResoureIdInDrawer);
     ...
     * @param activity
     */
    private void addStatusViewWithDrawable(Activity activity, Drawable drawable) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //占位状态栏
            View statusBarView = new View(activity);
            ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
