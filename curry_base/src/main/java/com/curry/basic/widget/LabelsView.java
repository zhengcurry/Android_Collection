...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    private boolean mModel;     //样式（true是固定列样式，false是瀑布流样式）add by curry
    private int mColumnsNumber; //列数 add by curry
    private int maxWidth;       //add by curry
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mModel = mTypedArray.getBoolean(R.styleable.labels_view_model, false);
            mColumnsNumber = mTypedArray.getInteger(R.styleable.labels_view_columnsNumber, 0);

            showDrawable = mTypedArray.getBoolean(R.styleable.labels_view_showDrawable, false);
            mShowDrawableId = mTypedArray.getResourceId(R.styleable.labels_view_showDrawableId, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //设置view 的最小宽度
            view.setMinimumWidth((maxWidth - mWordMargin * (mColumnsNumber - 1)) / mColumnsNumber);
            view.setMinimumHeight(mTextPaddingTop + mTextPaddingBottom + 27);
            ...
            //当前行显示不下item时换行。
            if (mModel) {
                if (mColumnsNumber != 0 && i % mColumnsNumber == 0) {//add by curry
                ...
                if (maxWidth < lineWidth + view.getMeasuredWidth()) { //add by curry
                    contentHeight += mLineMargin;
                    contentHeight += maxItemHeight;
                    maxItemHeight = 0;
                    maxLineWidth = Math.max(maxLineWidth, lineWidth);
        ...
        contentHeight += maxItemHeight;
        maxLineWidth = Math.max(maxLineWidth, lineWidth) + mWordMargin;//add by curry 2018年11月21日14:48:36
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mModel) {//固定列数
                ...
                if (contentWidth < x + view.getMeasuredWidth() + getPaddingRight()) {//add by curry
                    x = getPaddingLeft();
                    y += mLineMargin;
                    y += maxItemHeight;
                    maxItemHeight = 0;
...
//            }
    ...
    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider) {
        ...
        if (showDrawable) {
            Drawable drawable = mContext.getResources().getDrawable(mShowDrawableId);
            // 这一步必须要做,否则不会显示.
    ...
    public void selectDrawableStatus(TextView textView, int rId) {
     ...
     * @return
     */
    public ArrayList getSelectLabelId() {
        ArrayList arrayList = new ArrayList();
        if (getSelectLabelDatas() != null) {
            for (Object comboTypeBean : getSelectLabelDatas()) {
//                arrayList.add((() comboTypeBean).getValue());
            }
        }
        return arrayList;
    }
